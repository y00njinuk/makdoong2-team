...
/** 폴백 체인의 서열. 낮을수록 저렴·저성능. */
export type ModelTier = "low" | "medium" | "high" | "max";

/** opencode 의 reasoning variant. */
export type ModelVariant = "low" | "medium" | "high" | "xhigh" | "max";
...
export interface AgentModelPolicy {
  primary: ModelSpec;
  /** 순서대로 시도. 각 tier 는 primary.tier 보다 엄격히 낮아야 한다. */
  fallbacks: ModelSpec[];
}

/** 폴백 오버라이드 항목 — bare id(tier 는 "low" 기본) 또는 {id, tier?}. */
export type FallbackOverride = string | { id: string; tier?: ModelTier };

/** makdoong2-team.json `agents` 블록의 per-agent 오버라이드. */
...
/** makdoong2-team.json `model_policy` 블록. */
...
export interface NextModelInput {
  ...
  /** 방금 실패한 모델 id */
  current: string;
  /** 예: "rate_limit", "context_exceeded", "5xx" */
  reason?: string;
}
...
export interface BuildPoliciesResult {
  policies: Record<string, AgentModelPolicy>;
  allowed: Set<string>;
}
...
export const TIER_RANK: Record<ModelTier, number> = { low: 1, medium: 2, high: 3, max: 4 };
...
export const DEFAULT_POLICIES: Readonly<Record<string, AgentModelPolicy>> = Object.freeze({
...
function clonePolicies(src: Readonly<Record<string, AgentModelPolicy>>): Record<string, AgentModelPolicy> {
...
export function nextModel(
  { agent, current, reason }: NextModelInput,
  policies?: Readonly<Record<string, AgentModelPolicy>>,
): NextModelResult {
...
export function validatePolicies(
  policies: Readonly<Record<string, AgentModelPolicy>>,
  allowedPrimaries?: ReadonlySet<string>,
): void {
...
function normalizeFallback(entry: unknown): { id: string; tier: ModelTier } {
  if (typeof entry === "string") return { id: entry, tier: "low" };
  if (entry && typeof entry === "object" && typeof (entry as { id?: unknown }).id === "string") {
    const e = entry as { id: string; tier?: ModelTier };
 ...
 */
export function buildPoliciesFromConfig(
  { agents, model_policy }: {
    agents?: Record<string, AgentOverrideInput | undefined>;
    model_policy?: ModelPolicyOverrideInput;
  } = {},
): BuildPoliciesResult {
  ...
  if (model_policy && model_policy.allowed_primaries !== undefined) {
    ...
    if (!Array.isArray(model_policy.allowed_primaries)) {
      throw new Error(
        `model_policy.allowed_primaries 는 문자열 배열이어야 한다 ` +
