...
import type { ModelTier, NextModelResult } from "./model-policy.mts";
...
const assertThrows = (fn: () => void, matcher: RegExp | null, msg: string): void => {
...
const expectNext = (r: NextModelResult): NonNullable<NextModelResult["next"]> => {
  if (!r.next) throw new Error("expected a next model, got null (chain exhausted)");
  return r.next;
};
    ...
    () => buildPoliciesFromConfig({
      agents: {
        "makdoong2-engineer": {
          ...
          fallback_models: [{ id: "github-copilot/claude-haiku-4.5", tier: "ultra" as unknown as ModelTier }],
        },
      },
    }),
    /not a valid tier/,
