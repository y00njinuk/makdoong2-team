...
echo ""
echo "── stage-analysis-verify (requirements→analysis) ──"
# scope substage 흡수로 stage-analysis-verify.sh 의 승인·정합성 검사가 이 게이트로 이관됐다.
rm -f "$REPORT"
...
expect "minor 기본 auto-approve (승인 없이 통과)" pass stage-analysis-verify.sh
...
expect "major 기본도 요구사항 게이트는 무인 통과" pass stage-analysis-verify.sh
...
expect "구형 state 미승인 → 차단" block stage-analysis-verify.sh
...
expect "구형 state 사용자 승인 → 통과" pass stage-analysis-verify.sh
...
expect "인터뷰 미완료 → 차단 (auto와 무관)" block stage-analysis-verify.sh
...
mk '.stages."1_planning".substages."requirements".done=true | .stages."1_planning".substages."requirements".verification_pending=true | .stages."2_implementation".substages."analysis".done=true | .policy={category:"minor",auto_approve:{"1_planning.requirements":true}} | .worktree="'"$WT"'"'
expect "minor auto-approve → 통과" pass stage4-dev-verify.sh
mk '.stages."1_planning".substages."requirements".done=true | .stages."1_planning".substages."requirements".approved_by_user=false | .stages."2_implementation".substages."analysis".done=true | .worktree="'"$WT"'"'
expect "구형 state 미승인 → 차단" block stage4-dev-verify.sh
mk '.stages."1_planning".substages."requirements".done=true | .stages."1_planning".substages."requirements".approved_by_user=true | .stages."2_implementation".substages."analysis".done=true | .worktree="'"$WT"'"'
expect "구형 state 승인 → 통과" pass stage4-dev-verify.sh

echo ""
echo "── stage6-commit-verify (5→6) ──"
TESTS='.stages."1_planning".substages."jira".done=true | .stages."1_planning".substages."requirements".done=true | .stages."2_implementation".substages."dev".done=true | .stages."2_implementation".substages."test".unit="pass" | .stages."2_implementation".substages."test".integration="skip" | .stages."2_implementation".substages."test".coverage="exempt"'
