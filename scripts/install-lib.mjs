 ...
 */
function hardenSecretFile(path) {
    try {
...
import { cpSync, mkdirSync, existsSync, readFileSync, writeFileSync, chmodSync, readdirSync, rmSync, symlinkSync, lstatSync, renameSync, } from "node:fs";
...
function isLegacyPluginEntry(entry) {
    if (typeof entry !== "string")
        return false;
    if (entry === LEGACY_RELATIVE_PLUGIN_REF)
        return true;
    if (entry.endsWith(LEGACY_ABSOLUTE_TS_SUFFIX))
        return true;
    if (entry === LEGACY_SCOPED_NAME)
        return true;
    if (entry.startsWith(`${LEGACY_SCOPED_NAME}@`))
        return true;
    if (entry.startsWith(`${PLUGIN_PACKAGE_NAME}@`))
        return true;
    return false;
}
...
export function install(opts) {
    const { configDir, pkgRoot, force = false, patchOpencode = "idempotent", logger = console, } = opts;
    const result = {
        deployed: [],
        backedUp: [],
        skipped: [],
    ...
    if (existsSync(libSrc)) {
        ...
        for (const name of readdirSync(libSrc)) {
            ...
            if (name.endsWith(".sh"))
                chmodSync(join(libDst, name), 0o755);
        }
    }
...
function patchOpencodeJson(configDir, pkgRoot, strategy, { ok, info, warn, backup }) {
    ...
    if (existsSync(ocPath)) {
        try {
            ...
            throw new Error(`opencode.json could not be parsed (even allowing JSONC comments and trailing commas): ${ocPath}\n` +
                `  ${errMessage(err)}\n` +
        ...
        if (strategy === "idempotent") {
            ...
            const hasAllExtDirPaths = requiredExtDirPaths.every((p) => oc.permission?.external_directory?.[p] === "allow");
    ...
    if (!oc.plugin.includes(currentPluginRef))
        oc.plugin.push(currentPluginRef);
...
function removeLegacyReadPermission(oc) {
    ...
    if (!permission || !read || typeof read !== "object")
        return;
...
function ensureSecretsScaffolding(cfgPath, { ok, info }) {
    ...
    if (added === 0 && before && typeof before === "object")
        return;
...
function alwaysLoadIncludes(pluginArr, toolName) {
    ...
    if (!entry)
        return true; // tool-search not configured → all tools always visible
    const opts = entry[1];
...
function ensureToolsInAlwaysLoad(pluginArr, toolNames) {
    const entry = findToolSearchEntry(pluginArr);
    if (!entry)
        return; // tool-search not configured; nothing to patch
    if (!entry[1] || typeof entry[1] !== "object")
        entry[1] = {};
    if (!Array.isArray(entry[1].alwaysLoad))
        entry[1].alwaysLoad = [];
    for (const t of toolNames) {
        if (!entry[1].alwaysLoad.includes(t))
            entry[1].alwaysLoad.push(t);
    }
}
/**
...
function removeToolsFromAlwaysLoad(pluginArr, toolNames) {
    const entry = findToolSearchEntry(pluginArr);
    if (!entry)
        return false; // opencode-tool-search not present — nothing to do
    const opts = entry[1];
    if (!opts || typeof opts !== "object" || !Array.isArray(opts.alwaysLoad))
        return false;
...
function unpatchOpencodeJson(configDir, pkgRoot, { ok, skip, warn }) {
    ...
    if (Array.isArray(oc.plugin)) {
        ...
        if (oc.plugin.length < before)
            changed = true;
    }
    // — alwaysLoad (only when opencode-tool-search is configured) —
    if (Array.isArray(oc.plugin) && removeToolsFromAlwaysLoad(oc.plugin, TOOLS))
        changed = true;
    // — tools section —
    if (oc.tools && typeof oc.tools === "object") {
        ...
        if (Object.keys(oc.tools).length === 0)
            delete oc.tools;
    }
...
function removeOpencodeCache(pkgRoot, { ok, skip, warn }) {
    ...
    if (removedCount > 0)
        ok(`opencode plugin cache symlinks removed (${tags.join(", ")})`);
    else
        skip("no opencode plugin cache symlinks to remove");
}
/**
...
export function uninstall(opts) {
    const { configDir, pkgRoot, logger = console, } = opts;
    ...
    if (existsSync(agentsDir)) {
        ...
        for (const name of readdirSync(agentsDir)) {
            if (!name.startsWith("makdoong2-") || !name.endsWith(".md"))
                continue;
            rmSync(join(agentsDir, name), { force: true });
            count++;
        }
        if (count > 0)
            ok(`${count} agent definition(s) removed from agents/`);
        else
            skip("agents/: no makdoong2-*.md files found");
    }
    else {
    ...
    if (skillsRemoved > 0)
        ok(`${skillsRemoved} skill director(y/ies) removed`);
    else
        skip("skills/: no makdoong2-team skill directories found");
    ...
    if (existsSync(cmdDir)) {
        ...
        if (cmdRemoved > 0)
            ok(`${cmdRemoved} command file(s) removed from command/`);
        else
            skip("command/: no makdoong2-*.md files found");
    }
    else {
