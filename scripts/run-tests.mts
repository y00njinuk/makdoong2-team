...
const STEPS = [
  ...
  "node --test test/apply-patch-paths.test.ts",
  "node --test test/pipeline-shape.test.ts",
  "node --test test/permission-scope-hardening.test.ts",
...
// ── 1단계: 호스트 ──
const results: Array<{ step: string; code: number }> = [];
 ...
 */
function linuxCheckPlan(): { run: true } | { run: false; reason: string; loud: boolean } {
...
let linuxResult: { step: string; code: number } | null = null;
