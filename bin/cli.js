...
function doInstall(flags) {
    ...
    if (missing.length)
        warn(`missing runtime dependencies: ${missing.join(", ")}`);
    else
        ok("runtime dependencies (jq, git) present");
...
function doDoctor(flags) {
    ...
    let problems = 0;
    const check = (cond, good, bad) => { if (cond)
        ok(good);
    else {
        warn(bad);
        problems++;
    } };
    for (const b of ["jq", "git"])
        check(hasBinary(b), `${b} present`, `${b} missing on PATH`);
    const cfgPath = join(DEST, "makdoong2-team.json");
    let cfgOk = false;
    let cfgParsed = null;
    if (existsSync(cfgPath)) {
        try {
            cfgParsed = JSON.parse(readFileSync(cfgPath, "utf8"));
            cfgOk = true;
        }
        catch { /* invalid */ }
    }
    ...
    if (cfgOk) {
        ...
        check(modelOk, "model policy invariants pass", `model policy invalid: ${modelErr} (run: npx makdoong2-team validate)`);
    }
    ...
    if (existsSync(ocPath)) {
        try {
            ...
            pluginOk = Array.isArray(oc.plugin) && oc.plugin.some((p) => {
                if (matchesEntry(p))
                    return true;
                if (Array.isArray(p) && matchesEntry(p[0]))
                    return true;
                return false;
            });
    ...
    check(cachedVersion === null || cachedVersion === PKG.version, `opencode plugin cache matches installed version (v${PKG.version})`, `opencode plugin cache is stale: ${cachedDir} has v${cachedVersion} but this package is v${PKG.version} ` +
    ...
    if (legacySecrets.length === 0) {
        ...
        for (const p of legacySecrets.slice(0, 8))
            warn(`  ${p}`);
