...
| `researcher` | 리서치 fan-out 워커 | 소스 한 곳만 조사, 파일 쓰기 불가 |
...
조사가 여러 소스에 걸칠 때는 한 세션이 순서대로 훑지 않고 **소스마다 세션을 갈라 동시에** 조사한다 (`dispatch_research`). 두 가지를 동시에 얻기 위해서다 — 대기 시간이 직렬 합이 아니라 최장 소스 하나로 줄고, Jira 코멘트 전체·Confluence 원문·PR diff 가 **한 컨텍스트에 겹쳐 쌓이지 않는다**. 후자가 본질이다: 조사를 많이 할수록 판단할 컨텍스트가 좁아지는 역설이 lazy connect 만으로는 해소되지 않는다.
