...
describe("planner.md — early-exit guard for already-done substage (regression: PROJ-40406)", () => {
  ...
  test("each substage section (§1/§2) has a 선행 skip 체크", () => {
    const jiraSection = PROMPT.split("## §1. Substage: jira")[1]?.split("## §2.")[0] ?? "";
    const reqSection = PROMPT.split("## §2. Substage: requirements")[1]?.split("## 완료 조건")[0] ?? "";
  ...
  // scope substage 는 requirements 로 흡수됐다 (§3 삭제). 흡수 후에도 범위 확정
  // 여부는 requirements 의 self_check 4항목으로 판정되므로, §2 가 그 의무를
  // 명시하고 있는지를 대신 고정한다 — 명시가 빠지면 planner 가 범위 확정을
  // 통째로 건너뛰고 다음 게이트가 하드 차단한다.
  test("§2 가 흡수된 개발 범위 확정 의무를 명시한다", () => {
    const reqSection = PROMPT.split("## §2. Substage: requirements")[1]?.split("## 완료 조건")[0] ?? "";
    assert.match(reqSection, /개발 범위 확정/, "§2 에 개발 범위 확정 항목이 없다");
    assert.ok(
      !PROMPT.includes("## §3. Substage: scope"),
      "scope substage 섹션이 남아 있다 — STAGE_ORDER 에는 없는 단계를 planner 가 수행하려 한다",
    );
  });
