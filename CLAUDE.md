...
- **스코프 밖 요청은 abort 앞에 세션 안에서 먼저 되돌려 준다 (hardrule).** 관측된 위반은 우연이 아니라 모델이 저장소 안 검색이 비자 조부모를 검색 루트로 **명시적으로** 넘긴 것이었다 (`glob {path:"/root/IdeaProjects"}`, issue #12 재발). 재디스패치는 대화 이력과 진행 중 작업을 버리는 비싼 경로라 그 앞에 두 층을 둔다: ⓪ 모든 dispatch 프롬프트에 `buildPathScopePromptBlock()` 경로 범위 블록을 상시 주입 ① `outside_allowed_roots` 는 `reject` + `message`(피드백 거부)로 답한다 — opencode 는 이것을 `CorrectedError` 로 바꿔 툴 호출 하나만 실패시키고 **세션 루프는 계속 돌린다** (`RejectedError` 만 루프를 세운다). 예산은 `timeout.permission_corrections_per_session`(기본 3, 0 이면 종전 동작). 문구는 `POST /permission/{requestID}/reply` 로만 실리고 그 라우트는 디렉토리 스코프라 **`?directory=<worktree>` 를 명시**해야 한다 (세션-라우팅 엔드포인트는 `{response}` 만 받는다). `reject` 는 같은 세션의 나머지 대기 요청을 **연쇄 거부**하므로 허용을 먼저 전부 답한 뒤 거부 하나만 교정하고, 레지스트리의 그 세션 대기 목록은 선제로 비운다. 회귀: `test/poll-permission-scope.test.ts` 의 "issue #12 재발" 블록. 상세: ARCHITECTURE.md §4.2a.
